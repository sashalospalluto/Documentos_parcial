...
void informe_socioDeterminado(Socio arraySocio[],Prestamo arrayPrestamo[],Libro arrayLibro[],int size)
{
    int i;
    int j;
    int opcion;
    ...
    for(i=0;i<size;i++)
    {
        ...
        {
            printf("\n %d- %s %s",i,arraySocio[i].nombre, arraySocio[i].apellido);
        }
    }
    utn_getUnsignedInt("\nIngrese el socio para listar los libros: ","error",0,sizeof(int),0,size,3,&opcion);
    printf("\nLibros que solicito el socio %s %s\n",arraySocio[opcion].nombre,arraySocio[opcion].apellido);
    for(j=0; j<size;j++)
    {
        if((arraySocio[opcion].idUnico==arrayPrestamo[j].codigoSocio) && (arrayPrestamo[j].isEmpty==0))
        {
            libro_buscarID(arrayLibro,size,arrayPrestamo[j].codigoLibro,&posLibro);
            printf("\n- %s",arrayLibro[posLibro].titulo);
        }
    }
...
void informe_mostrar(Autor arrayAutor[], Libro arrayLibro[], Socio arraySocio[], Prestamo arrayPrestamo[], int size)
{
    ...
    do{
        ...
        case 3:
            ...
            break;
        case 4:
            informe_socioDeterminado(arraySocio,arrayPrestamo,arrayLibro,size);
        case 5:
            break;
        case 11: //salir
            break;
